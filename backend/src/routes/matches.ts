  ...
  try {
    ...
    const pairMeal = await prisma.pairMeal.create({
      data: {
        ...
        placeGooglePlaceId:
          payload.googlePlaceId != null ? payload.googlePlaceId.trim() : null,
        restaurantName:
          payload.restaurantName != null ? payload.restaurantName.trim() : null,
        restaurantAddress:
          payload.restaurantAddress != null ? payload.restaurantAddress.trim() : null,
  ...
  try {
    ...
    const updated = await prisma.pairMeal.update({
      ...
      data: {
        ...
        placeLatitude:
          payload.latitude !== undefined ? payload.latitude : pairMeal.placeLatitude,
        placeLongitude:
          payload.longitude !== undefined ? payload.longitude : pairMeal.placeLongitude,
        placeGooglePlaceId:
          payload.googlePlaceId != null
            ? payload.googlePlaceId.trim()
            : pairMeal.placeGooglePlaceId,
        restaurantName:
          payload.restaurantName != null
            ? payload.restaurantName.trim()
            : pairMeal.restaurantName,
        restaurantAddress:
          payload.restaurantAddress != null
...
function parseMeetingTimeToMinutes(value: string | null | undefined): number | null {
  if (value == null) {
    return null;
  }
