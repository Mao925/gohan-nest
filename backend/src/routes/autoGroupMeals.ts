...
import {
  getAvailabilitySplitForSlot,
  type AutoGroupCandidate
} from '../utils/groupMealAutoCandidates.js';
...
async function getTargetCommunityIds(): Promise<string[]> {
  const communities = await prisma.community.findMany({ select: { id: true } });
  return communities.map((community) => community.id);
}

const MEAL_TIME_SLOTS: MealTimeSlot[] = [MealTimeSlot.LUNCH, MealTimeSlot.DINNER];

async function createRealGroupMealsForCommunity(
  communityId: string,
  date: Date
): Promise<void> {
  const weekday = getWeekdayForDate(date);
  const meetingDate = startOfDayForDate(date);
  const expiresCache: Record<MealTimeSlot, Date> = {
  ...
  for (const mealTimeSlot of MEAL_TIME_SLOTS) {
    ...
    for (const group of groups) {
      const groupMembers = group.userIds
        ...
        .filter(
          (candidate): candidate is AutoGroupCandidate => Boolean(candidate)
        );
...
async function createMeetGroupMealsForCommunity(
  communityId: string,
  date: Date
): Promise<void> {
  const weekday = getWeekdayForDate(date);
  ...
  for (const mealTimeSlot of MEAL_TIME_SLOTS) {
    ...
    const meetCandidates =
      realCandidateCount >= 2 ? meetOnly : [...available, ...meetOnly];
...
autoGroupMealsRouter.post('/real', async (req, res) => {
  try {
    ...
    await Promise.all(
      communityIds.map((communityId) => createRealGroupMealsForCommunity(communityId, today))
    );
...
autoGroupMealsRouter.post('/meet', async (req, res) => {
  try {
    ...
    const communityIds = await getTargetCommunityIds();
    await Promise.all(
      communityIds.map((communityId) => createMeetGroupMealsForCommunity(communityId, today))
    );
    return res.status(204).send();
  } catch (error) {
