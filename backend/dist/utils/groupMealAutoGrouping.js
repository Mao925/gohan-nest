...
function getRelationshipWeight(a, b, likeSet) {
...
function computeProfileSimilarity(a, b) {
    ...
    const sharedHobbies = (profileA.hobbies ?? []).filter((hobby) => (profileB.hobbies ?? []).includes(hobby));
    score += Math.min(sharedHobbies.length, 3);
    const sharedMeals = (profileA.favoriteMeals ?? []).filter((meal) => (profileB.favoriteMeals ?? []).includes(meal));
    score += Math.min(sharedMeals.length, 2);
    return score;
}
function computeAffinity(a, b, likeSet) {
...
export async function groupCandidatesIntoBoxes(params) {
    ...
    while (pool.length > 0) {
        ...
        pool.sort((a, b) => computeAffinity(b, seed, likeSet) - computeAffinity(a, seed, likeSet));
            ...
            if (singletonCandidate) {
                ...
                for (let i = 0; i < groups.length; i += 1) {
                    ...
                    if (!existingMembers.length)
                        continue;
                    const affinitySum = existingMembers.reduce((sum, member) => sum + computeAffinity(singletonCandidate, member, likeSet), 0);
