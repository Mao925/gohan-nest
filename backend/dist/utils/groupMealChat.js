...
export async function assertCanAccessGroupMealChat(userId, groupMealId) {
    ...
    if (!membership) {
        throw new GroupMealChatAccessError(403, 'コミュニティ参加が承認されていません');
    }
    ...
    if (!participant) {
        throw new GroupMealChatAccessError(403, 'この箱に参加していません');
    }
    if (!ATTENDING_PARTICIPANT_STATUSES.includes(participant.status)) {
        throw new GroupMealChatAccessError(403, '招待段階または参加が確定していないため、チャットに参加できません');
    }
    return groupMeal;
}
