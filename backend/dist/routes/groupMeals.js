...
function buildSchedulePayloadFromGroupMeal(groupMeal) {
    ...
    return {
        ...
        meetingTime: meetingTimeMinutes !== null
...
async function syncGroupMealStatus(db, groupMealId, capacity, currentStatus, hostUserId) {
...
groupMealsRouter.post("/", async (req, res) => {
    ...
    const { title, capacity, budget, schedule, mode: rawMode, meetUrl: rawMeetUrl, } = parsed.data;
    const normalizedMode = rawMode === GroupMealMode.MEET ? GroupMealMode.MEET : GroupMealMode.REAL;
...
groupMealsRouter.get("/", async (req, res) => {
    ...
    const now = new Date();
    const modeParam = typeof req.query.mode === "string"
        ? req.query.mode.toUpperCase()
        : undefined;
    const modeFilter = modeParam === "REAL"
        ? GroupMealMode.REAL
        : modeParam === "MEET"
            ? GroupMealMode.MEET
            : undefined;
...
groupMealsRouter.get("/:groupMealId", async (req, res) => {
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            const remainingCapacity = await getGroupMealRemainingCapacityTx(tx, groupMealId, { precomputedActiveCount: headcount });
...
groupMealsRouter.post("/:id/invite", async (req, res) => {
    ...
    const countedActiveParticipants = getCountedParticipantsForGroupMeal(groupMeal, ACTIVE_PARTICIPANT_STATUSES);
...
groupMealsRouter.patch("/:groupMealId/participant/status", async (req, res) => {
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            await syncGroupMealStatus(tx, groupMeal.id, groupMeal.capacity, groupMeal.status, groupMeal.hostUserId);
        });
        const updated = await fetchGroupMeal(groupMealId);
        return res.json(buildGroupMealPayload(updated, req.user.userId));
    }
...
groupMealsRouter.post("/:id/join", async (req, res) => {
    ...
    const activeCount = getCountedParticipantsForGroupMeal(groupMeal, ACTIVE_PARTICIPANT_STATUSES).length;
