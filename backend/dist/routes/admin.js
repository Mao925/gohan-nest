...
adminRouter.get("/communities/:communityId/users/:fromUserId/hearts", async (req, res) => {
...
adminRouter.get("/communities/:communityId/users/:fromUserId/stars", async (req, res) => {
    const { communityId, fromUserId } = req.params;
    const community = await prisma.community.findUnique({
        where: { id: communityId },
    });
...
function buildNamePayload(user) {
    const name = user.profile?.name || user.lineDisplayName || user.email || "Unknown User";
